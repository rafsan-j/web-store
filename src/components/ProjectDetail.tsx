...
import { X, GitBranch, ExternalLink, Globe, Lock, Calendar, Activity, Trash2, Edit3 } from "lucide-react";
...
export default function ProjectDetail({ project, onClose, onDelete }: Props) {
        ...
        <Section label="Health Score">
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <div style={{ flex: 1, height: "6px", borderRadius: "3px", background: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
              <div style={{ width: `${health}%`, height: "100%", background: healthColor, borderRadius: "3px", transition: "width 0.4s ease" }} />
            </div>
            <span style={{ fontSize: "18px", fontFamily: "'Instrument Serif', serif", color: healthColor, minWidth: "40px" }}>
              {health}
            </span>
          </div>
        </Section>
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: "24px" }}>
      <p style={{ fontSize: "9px", letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--text-muted)", marginBottom: "10px", fontWeight: 500 }}>
        {label}
      </p>
      {children}
    </div>
